...
| Angular web app — including the AuthService/JWT interceptor/route guard and the real post-login Freelancer/Client dashboards (login, contracts, milestones, escrow actions, tax summary, job search/posting) | **Builds and visually verified.** `ng build` succeeds; both dashboards and the login flow were checked with a local mock API server (matching DashboardController's/AuthController's exact JSON contracts) driven by Playwright, across a desktop and an iPhone SE viewport. Two real bugs were found and fixed this way — an `@if`/`as` control-flow expression only valid on the primary block, and a narrow-viewport (320px) horizontal overflow from unbreakable rupee amounts in a 2-column stat grid. | — |
| Flutter mobile app — including the auth-aware bottom nav (Login ↔ Dashboard tab), AuthService (JWT session persisted via `shared_preferences`), and the Freelancer/Client dashboard screens mirroring the web app's | Source-complete Dart (freelancer/client signup, jobs list, login, both dashboards, validators incl. a new GSTIN check), **not built, analyzed, or run here** — same reason as before: the Flutter SDK, `pub.dev`, and `dl.google.com`/`maven.google.com` are all unreachable from this sandbox. New unit tests (`test/services/dashboard_models_test.dart`) parse realistic DashboardController/AuthController JSON fixtures and round-trip AuthService's persisted-session logic (via `SharedPreferences.setMockInitialValues`), so at least the JSON-contract and session-expiry logic is exercised the moment `flutter test` can run — which is CI, not here. | `.github/workflows/flutter.yml` builds and tests it for real on push — see §7.2. Or run `flutter create --platforms=android .` in `mobile/verqo_mobile` with a real install. |
