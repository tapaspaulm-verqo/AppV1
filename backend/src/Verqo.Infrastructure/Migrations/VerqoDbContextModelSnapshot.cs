...
/// <summary>
/// The live snapshot `dotnet ef migrations add` diffs the current
/// VerqoDbContext model against to compute the next migration. Hand-written
/// alongside 20260922000000_InitialCreate.cs and its Designer.cs for the
/// same NuGet-access reason — see that migration's remarks. Deliberately
/// sticks to the oldest, most stable slice of the ModelBuilder fluent API
/// (Property&lt;T&gt;(string), HasKey, HasIndex, ToTable, HasOne/WithMany/
/// WithOne, HasForeignKey, OnDelete, Navigation) rather than
/// version-specific annotation helpers, since a compile error here would
/// break the whole Verqo.Infrastructure build — the one outcome worse than
/// this snapshot being imperfect. The CI job in .github/workflows/dotnet.yml
/// runs `dotnet ef migrations add __ConsistencyCheck` against a real
/// Postgres service container and asserts EF reports no pending model
/// changes, which is the real check on whether this file actually matches
/// VerqoDbContext — treat that as the source of truth over this comment.
/// </summary>
[DbContext(typeof(VerqoDbContext))]
partial class VerqoDbContextModelSnapshot : ModelSnapshot
{
